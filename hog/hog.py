...
def is_swap(player_score, opponent_score):
    ...
    swap_num = 3 ** sum_scores
    if swap_num < 10:
        return True
    else:
        last_digit = swap_num % 10

        while swap_num >= 10:
            swap_num = swap_num // 10
        first_digit = swap_num
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            rolls_0 = strategy0(score0, score1)
            new_points_0 = take_turn(rolls_0, score1, dice)
            if abs(last_points_0 - rolls_0) == 2 and feral_hogs:
                score0 += 3
            score0 += new_points_0
            last_points_0 = new_points_0
        else:
            rolls_1 = strategy1(score1, score0)
            new_points_1 = take_turn(rolls_1, score0, dice)
            if abs(last_points_1 - rolls_1) == 2 and feral_hogs:
                score1 += 3
            score1 += new_points_1
            last_points_1 = new_points_1
...
def make_averaged(g, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def compute_calls(*args):
        num_calls = 0
        call_counter = 0
        while num_calls < num_samples:
            call_counter += g(*args)
            num_calls += 1
        return call_counter / num_samples
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    best_rolls = 1
    best_points = 0
    while num_roll_counter <= 10:
        avg_points = test_func(num_roll_counter, dice)
        if avg_points > best_points:
            best_rolls = num_roll_counter
